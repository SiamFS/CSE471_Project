...
const AddToPayment = () => {
  ...
  const handleRemoveItem = (id) => {
    fetch(`https://cse471-project-backend.onrender.com/cart/${id}`, {
      method: 'DELETE',
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      const response = await fetch('https://cse471-project-backend.onrender.com/create-checkout-session', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setCheckoutWarning(error.message || 'Payment processing failed. Please try again.');
    }
  };
  ...
  const handleCashOnDelivery = async () => {
    ...
    const response = await fetch('https://cse471-project-backend.onrender.com/cash-on-delivery', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    <div className='pt-28 px-4 lg:px-24'>
      <div className='flex flex-col lg:flex-row'>
        ...
        <div className='lg:w-1/4 p-4 border rounded shadow-lg lg:ml-4'>
          ...
          {checkoutWarning && (
            <p className='mt-2 text-red-600'>{checkoutWarning}</p>
          )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg max-w-md w-full">
            ...
            {/* Address Form for Cash on Delivery */}
            {paymentMethod === 'cash' && (
              <div className="mt-4">
                <input
                  type="text"
